...
def manejar_cliente(conn, addr):
    ...
    try:
        ...
        while True:
            ...
            # ── detectar comandos ANTES de intentar parsear como pedido ──
            # los comandos empiezan con '/' → se identifican en la primera letra
            # si no es comando, se procesa como pedido normal
            if datos.startswith('/'):
                comando = datos.lower()

                if comando == '/stock':
                    # leer stock dentro del lock → valor confiable
                    # otro hilo podría estar modificando stock{} en este momento
                    with lock_pedidos:
                        # construir respuesta legible línea por línea
                        lineas = ["\n📦 STOCK ACTUAL:"]
                        for prod, cant in stock.items():
                            # barra visual proporcional a la cantidad
                            barra = "█" * (cant // 5) if cant > 0 else "░ sin stock"
                            lineas.append(f"  {prod:12s} → {cant:3d} unidades  {barra}")
                        lineas.append(f"  Cola actual: {len(pedidos)}/{MAX_COLA} pedidos\n")
                        respuesta_stock = "\n".join(lineas)
                    conn.sendall(respuesta_stock.encode())
                    print(f"[SERVIDOR] {nombre_cliente} consultó el stock")

                elif comando == '/help':
                    # lista de comandos disponibles
                    ayuda = (
                        "\n📋 COMANDOS DISPONIBLES:\n"
                        "  /stock  → consultar stock actual\n"
                        "  /help   → mostrar esta ayuda\n"
                        "  producto,cantidad → hacer un pedido\n"
                        "  Ejemplo: productoA,3\n"
                    )
                    conn.sendall(ayuda.encode())

                else:
                    conn.sendall(f"⚠️  Comando '{datos}' desconocido. Escribe /help\n".encode())

                # después de procesar el comando NO continuar al bloque de pedidos
                continue

            # ── parsear mensaje como pedido normal ──
            try:
                producto, cantidad = datos.split(',')
                cantidad = int(cantidad.strip())
                producto = producto.strip()
            except ValueError:
                conn.sendall("❌ Formato incorrecto. Usa: producto,cantidad  o  /help\n".encode())
                continue
