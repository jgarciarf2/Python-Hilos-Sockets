...
================================================================================
MODIFICACIÓN 8: CLIENTE CON CANTIDAD FIJA DE PEDIDOS
================================================================================

PROPÓSITO GENERAL DEL ARCHIVO:
    - Implementar un SERVIDOR TCP que actúa como una "central de pedidos".
    - Los CLIENTES (productores) se conectan remotamente vía socket TCP/IP y
      envían pedidos de productos.
    ...
    - Se usan todas las primitivas de sincronización vistas en clase:
        ...
        6. BARRERA         → los procesadores esperan a que haya al menos N
                             pedidos acumulados antes de empezar a despachar.

NOTA SOBRE ESTA MODIFICACIÓN:
    - El SERVIDOR NO cambia respecto al original.
    - Todo el cambio de la Modificación 8 está en el CLIENTE.
    - El servidor simplemente recibe los pedidos que el cliente le envía,
      sin importar si el número de pedidos fue elegido aleatoriamente o por
      el usuario.
    - El protocolo JSON sobre TCP permanece idéntico.
...
#       → Crea un hilo que ejecutará la función 'funcion' con los argumentos.
...
def mostrar_stock():
    ...
    with lock_stock:
        ...
        for producto, cantidad in stock_productos.items():
            # f-string con formato: {producto:<15} alinea a la izquierda en 15 chars.
...
def procesar_pedido(pedido):
    ...
    Proceso:
        1. Adquiere el LOCK del stock para leer/modificar el inventario.
        2. Verifica si hay suficiente stock del producto solicitado.
        3. Si hay stock: descuenta la cantidad y marca como "DESPACHADO".
        4. Si NO hay stock: marca como "RECHAZADO" (sin stock).
        5. Suelta el lock.
        6. Simula un tiempo de procesamiento aleatorio (1-5 segundos).
           Esto se hace FUERA del lock para no bloquear a otros hilos
           innecesariamente mientras se "espera".
    """
...
def atender_cliente(conexion_cliente, direccion_cliente, id_cliente):
    ...
    try:
        ...
        mensaje_bienvenida = {
            "tipo": "bienvenida",                      # Tipo de mensaje.
            "mensaje": f"Bienvenido {nombre_cliente} a la Central de Pedidos",
            "productos_disponibles": lista_productos,  # Lista de nombres.
        ...
        # sendall() → envía TODOS los bytes (a diferencia de send(), que puede
        #             enviar solo una parte y habría que reintentar).
        ...
        while True:
            ...
            try:
                ...
                # Verificar si el cliente envió la señal de FIN.
...
def iniciar_servidor():
    ...
    print("   MOD 8: Cliente con Cantidad Fija de Pedidos")
    print("=" * 60)
    mostrar_stock()
    ...
    for i in range(1, NUM_PROCESADORES + 1):
        ...
        # args: tupla con los argumentos. (i,) es una tupla de un elemento
        #       (la coma es necesaria para que Python la reconozca como tupla).
    ...
    if not evento_barrera_liberada.is_set():
        log("⚠ No se alcanzaron suficientes pedidos para la barrera. "
            "Abortando barrera...")
